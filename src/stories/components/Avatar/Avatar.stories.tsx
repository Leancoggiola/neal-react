import { Meta, StoryObj } from "@storybook/react";
import { NlAvatar } from "../../../components";
import { actionIcFace } from "../../../assets/icons";

const meta: Meta<typeof NlAvatar> = {
  title: 'Components/Avatar',
  component: NlAvatar,
  argTypes: {
    // Behaviour
    src: {
      description: 'URL for avatar image or icon',
      value: '',
      type: { name: 'string', required: false }
    },
    userName: {
      description: 'Use on description and formatted as initials if src not provided',
      value: '',
      type: { name: 'string', required: false }
    },
    description: {
      description: 'Alterante text for image',
      value: '',
      type: { name: 'string', required: false }
    },
    interactive: {
      description: 'Define is avatar renders as button or div',
      value: '',
      type: { name: 'boolean', required: false }
    },
    // Styles
    size: {
      description: 'Specifies the size of avatar',
      value: 'small',
      type: { name: 'string', required: false },
      options: ['small', 'medium', 'large'],
      control: { type: 'radio' }
    },
    status: {
      description: 'Specifies the status of avatar',
      value: 'online',
      type: { name: 'string', required: false },
      options: ['online', 'no-disturb', 'no-available', 'absent', 'warning', 'offline'],
      control: { type: 'radio' }
    },
    variant: {
      description: 'Specifies the variant style of avatar',
      value: 'image',
      type: { name: 'string', required: false },
      options: ['image', 'icon'],
      control: { type: 'radio' }
    },
    disabled: {
      description: 'Specifies if avatar is disabled when interactive',
      value: false,
      type: { name: 'boolean', required: false },
    },
    className: {
      description: 'Custom class to add or modified to the component styles.',
      value: '',
      type: { name: 'string', required: false },
    },
  },
  decorators: [
    Story => (
      <div style={{ padding: '12px' }}>
        <Story />
      </div>
    ),
  ],
};

export default meta;
type Story = StoryObj<typeof NlAvatar>;

export const AvatarDefault: Story = {
  render: (args) => {
    const src = args.variant === 'icon' ? actionIcFace : 'data:image/jpeg;base64,';
    return <NlAvatar {...args} src={src} />
  },
  args: {
    size: 'medium',
    variant: 'image',
    userName: 'Test User',
    description: 'Test User avatar'
  },
  parameters: {
    controls: {
      exclude: ["src"]
    }
  }
};

export const AvatarInitials: Story = {
  args: {
    size: 'large',
    variant: 'image',
    userName: 'User Test',
    description: 'Uset test avatar initial',
    status: 'no-disturb',
    interactive: true
  },
  parameters: {
    controls: {
      exclude: ["src"]
    }
  }
};